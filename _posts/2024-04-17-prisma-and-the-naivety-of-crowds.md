...
Given the likelihood of accidentally undefined values, even with the compile time protection of TypeScript, the potential consequence of this design decision is terrifying. The issue was reported in July 2023 and Prisma's developers agree that the behaviour is dangerous, but have no plans to address it. Prisma's `updateMany` suffers from the same problem.
